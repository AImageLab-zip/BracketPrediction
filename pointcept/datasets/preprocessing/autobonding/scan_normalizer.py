...
scan_normalizer.py
------------------
Normalization pipeline for intra-oral scans loaded with trimesh.
...
upper:
  transforms:
    - type: translate
      values: [tx, ty, tz]
    - type: rotate
      axis: [ax, ay, az]   # will be unit-normalised automatically
      angle_deg: 45.0
    - type: scale
      values: [sx, sy, sz]  # uniform: scalar; non-uniform: list of 3
    - type: matrix
      values:               # row-major 4x4 matrix (list of 16 floats)
        - [m00, m01, m02, m03]
        - [m10, m11, m12, m13]
        - [m20, m21, m22, m23]
        - [m30, m31, m32, m33]
...
lower:
  transforms:
    - type: translate
      values: [0, 0, -5]
-----------------------------------
Supported transform types: translate | rotate | scale | matrix
"""
...
def _build_translation_matrix(values: Sequence[float]) -> np.ndarray:
...
def _build_rotation_matrix(
    ...
    """Return a 4×4 homogeneous rotation matrix (axis-angle)."""
    ...
    # Rodrigues' rotation formula expanded to a 3×3 matrix
    R3 = np.array([
...
def _build_scale_matrix(values: float | Sequence[float]) -> np.ndarray:
    """Return a 4×4 homogeneous scale matrix."""
    mat = np.eye(4, dtype=float)
    if np.isscalar(values):
        mat[0, 0] = mat[1, 1] = mat[2, 2] = float(values)
    else:
        values = list(values)
        if len(values) != 3:
            raise ValueError("Scale 'values' must be a scalar or a list of 3.")
        mat[0, 0], mat[1, 1], mat[2, 2] = values
    return mat


def _parse_transform(cfg: dict) -> np.ndarray:
    """Convert a single transform config dict to a 4×4 numpy matrix."""
    t_type = cfg.get("type", "").lower()

    if t_type == "translate":
        return _build_translation_matrix(cfg["values"])

    elif t_type == "rotate":
        return _build_rotation_matrix(cfg["axis"], cfg["angle_deg"])

    elif t_type == "scale":
        return _build_scale_matrix(cfg["values"])

    elif t_type == "matrix":
        mat = np.array(cfg["values"], dtype=float)
        if mat.shape != (4, 4):
            raise ValueError("'matrix' transform must be a 4×4 matrix.")
        return mat
